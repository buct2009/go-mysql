...
func (i Interval) StringHard() string {
	if i.Stop == i.Start+1 {
...
func (s *UUIDSet) StringHard() string {
	return hack.String(s.BytesHard())
}
...
func (s *MysqlGTIDSet) UpdateGTIDSet(GTID GTIDSet) error {
	for _, uuidSet := range GTID.(*MysqlGTIDSet).Sets {
		s.AddSet(uuidSet)
	}
	return nil
}
...
func (s *MysqlGTIDSet) StringHard() string {
	// there is only one element in gtid set
	if len(s.Sets) == 1 {
		for _, set := range s.Sets {
			return set.StringHard()
		}
	}
	...
	sets := make([]string, 0, len(s.Sets))
	for _, set := range s.Sets {
		sets = append(sets, set.StringHard())
	}
	sort.Strings(sets)
